...
void encrypt(public_key *pub, mpz_t cleartext, mpz_t ciphertext) {
	...
	// Chiffrement RSA : ciphertext = (cleartext^e) mod n
    mpz_powm(ciphertext, cleartext, pub->e, pub->n);

}

void decrypt(private_key *priv, mpz_t ciphertext, mpz_t cleartext) {
    
    // Déchiffrement RSA : cleartext = (ciphertext^d) mod n
    mpz_powm(cleartext, ciphertext, priv->d, priv->n);
